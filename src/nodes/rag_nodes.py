...
async def reference_docs_faiss_node(state: AgentState) -> Dict[str, Any]:
    ...
    if faiss_context:
        sep = prompt_service.render_template("rag_messages.jinja2", type="context_separator")
        reference_docs = f"{faiss_context}{sep}{reference_docs}"
...
async def reference_docs_pinecone_node(state: AgentState) -> Dict[str, Any]:
    ...
    if state.get("expanded_queries"):
        query = state['expanded_queries'][0]['query']
        logger.info(f"Utilizing Expanded Query for Pinecone: {query}")
    
    # Call actual Pinecone search
    retrieved_results = await pinecone_service.search(query, top_k=5)
    ...
    if retrieved_results:
        ...
        logger.info(f"Retrieved {len(retrieved_results)} snippets from Pinecone")
    ...
    # Render with Pinecone context
    reference_docs = prompt_service.render_template("reference_document.jinja2", **context_parts)
    if pinecone_context:
        sep = prompt_service.render_template("rag_messages.jinja2", type="context_separator")
        reference_docs = f"{pinecone_context}{sep}{reference_docs}"
